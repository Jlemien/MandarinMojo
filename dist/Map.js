...
game.Map.prototype._drawTile = function (x, y, drawWidth, drawHeight, layerName) {
    var layer = null, imageindex = null, sourceWidth = null, sourceX = null, sourceY = null;
    ...
    for (var i in this.jsonobj.layers) {
        ...
        gContext.drawImage(img, sourceX, sourceY, this.jsonobj.tilewidth, this.jsonobj.tileheight, drawX, drawY, drawWidth, drawHeight);
    }
    if (!layerFound) {
...
game.Map.prototype.drawLabels = function () {
    for (var i in this.labels) {
        label = this.labels[i];
        drawText(gContext, label.text, gWorld.textsize, gWorld.textcolor, label.pos[0] - this.cameraposition[0], label.pos[1] - this.cameraposition[1]);
    }
};
