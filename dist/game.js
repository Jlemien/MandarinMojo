...
(function () {
    ...
    window.onload = function () {
        ...
        if (util.getParameterByName("Pinyin") == 1) {
            gPinyin = dc("p");
            text = document.createTextNode(" ");
            gPinyin.appendChild(text);
            gLeft.appendChild(gPinyin);
        }
        if (util.getParameterByName("Audio") == 1) {
            gAudio = dc("span");
            $("foot").appendChild(gAudio);
        }
        if (util.getParameterByName("English") == 1 || (!gPinyin && !gAudio)) {
            gQuestion = dc("p");
    ...
    function loadWords() {
        ...
        if (util.getParameterByName("traditional") == 1) {
            traditionaloffset = 1;
        }
        var squiglytoneoffset = 1; // hē
        if (util.getParameterByName("tone") == 1) {
            ...
            if (xmlhttp.status == 200 && xmlhttp.readyState == 4) {
                ...
                for (var j = 0; j < words.length; j += 5) {
                    if (words[j] && words[j + traditionaloffset] != "") {
                        wordobjects[i].push(new game.Word(words[j + traditionaloffset], words[j + 2 + squiglytoneoffset], squiglytoneoffset == 1 ? words[j + 2] : null, words[j + 4], true));
                    }
                }
            }
            //}
    ...
    function drawGame() {
        ...
        if (gWorld.debug) {
            ...
            drawText(gContext, frames, gWorld.textsize, gWorld.textcolor, 150, 0);
        }
    }
